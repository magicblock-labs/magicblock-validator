...
async fn test_create_rent_pending_ata_zero_balance_rolls_back_for_privileged_payer(
    ...
    // Post-delegation actions execute with the validator as fee payer; the
    // funding invariant must still apply to them.
    let mut payer_acc = env.accountsdb.get_account(&payer).unwrap();
