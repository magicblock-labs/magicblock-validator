...
// NOTE: Flow "Account created then fetched, then delegated"
#[tokio::test]
async fn test_deleg_after_subscribe_case2_compressed() {
    let mut slot: u64 = 11;
    ...
    // even before the ensure_writable request
    {
        info!("3. Delegate account {pubkey} to us");
        ...
        // Compressed accounts don't need the PDA to exist
        rpc_client.remove_account(&pubkey);
        let updated = ctx
