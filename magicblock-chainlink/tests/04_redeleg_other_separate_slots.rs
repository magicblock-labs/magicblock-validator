...
async fn test_undelegate_redelegate_to_other_in_separate_slot_compressed() {
    ...
    let program_pubkey = Pubkey::new_unique();
    let other_authority = Pubkey::new_unique();
    let acc = Account::new(0, 0, &program_pubkey);
    ...
    // Undelegation requested, setup subscription, writes refused
    {
        ...
        let undelegated_acc = ctx
            .commit_and_undelegate_compressed(&pubkey, &program_pubkey)
            .await
            .unwrap();

        // Account should be empty onchain
        assert_eq!(cloner.get_account(&pubkey).unwrap(), undelegated_acc);
    ...
    // Delegate to other, subscription update, writes refused
    {
        ...
        // Compressed accounts don't send notifications when they are undelegated.
        ...
        // Logical state matches; `compressed` stays set (expanded from `CompressedDelegationRecord`).
        let mut acc_redeleg_expected = account_shared_with_owner_and_slot(
            &acc.account,
            program_pubkey,
            slot,
        );
        acc_redeleg_expected.set_compressed(true);
        assert_eq!(cloner.get_account(&pubkey).unwrap(), acc_redeleg_expected);
