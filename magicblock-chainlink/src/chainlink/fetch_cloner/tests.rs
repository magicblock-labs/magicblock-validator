...
#[tokio::test]
async fn test_build_existing_subs_keeps_watched_unmaterialized_delegation_record(
) {
    let validator_keypair = Keypair::new();
    let validator_pubkey = validator_keypair.pubkey();
    let account_pubkey = random_pubkey();
    ...
    let existing_subs =
        pipeline::build_existing_subs(&fetch_cloner, &[account_pubkey])
            .existing_subs;

    assert!(existing_subs.contains(&deleg_record_pubkey));
}

#[tokio::test]
async fn test_fetch_and_clone_delegated_account_keeps_existing_delegation_record_sub(
) {
    let validator_keypair = Keypair::new();
    let validator_pubkey = validator_keypair.pubkey();
    let account_pubkey = random_pubkey();
