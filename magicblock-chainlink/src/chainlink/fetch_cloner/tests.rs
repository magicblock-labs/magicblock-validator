...
async fn test_fetch_and_clone_single_non_delegated_account() {
    ...
    let FetcherTestCtx {
        ...
        fetch_cloner,
        ..
    } = setup([(account_pubkey, account.clone())], 100, validator_keypair.insecure_clone()).await;
...
async fn test_allowed_programs_filters_programs() {
    ...
    let FetcherTestCtx {
        ...
        remote_account_provider,
        ..
    } = setup(setup_accounts.into_iter(), CURRENT_SLOT, validator_keypair.insecure_clone()).await;
