...
```rust
use magicblock_configuration::MagicBlockParams;
...
fn main() -> Result<(), figment::Error> {
    ...
    let config = MagicBlockParams::try_new(args)?;
...
Environment variables are mapped using the `figment` provider.
  ...
  * **Prefix**: `MBV_`
  * **Separator**: Use a single underscore `_` to denote hierarchy (nesting).
  * **Field Names**: Use `kebab-case` (preserve dashes) for the actual field names.

### Examples

| Struct Field | Config Section | Environment Variable |
| :--- | :--- | :--- |
| `validator.basefee` | `[validator]` | `MBV_VALIDATOR_BASEFEE` |
| `ledger.block_time` | `[ledger]` | `MBV_LEDGER_BLOCK-TIME` |
| `chain_operation.country_code` | `[chain-operation]` | `MBV_CHAIN-OPERATION_COUNTRY-CODE` |
...
```bash
cargo test -p magicblock-configuration
```
