...
use magicblock_chainlink::{
    ...
    AccountFetchContext, InnerChainlink, ReplicationModeAwareChainlink,
};
use magicblock_config::config::{ChainLinkConfig, LifecycleMode};
use solana_account::{Account, AccountSharedData};
use solana_pubkey::Pubkey;
use solana_sdk::{
    clock::Slot, signature::Signer, signer::keypair::Keypair, sysvar::clock,
};
use tokio::sync::mpsc;
use tracing::*;
...
impl TestContext {
    pub async fn init(slot: Slot) -> Self {
        Self::init_with_risk_service(slot, None).await
    }
