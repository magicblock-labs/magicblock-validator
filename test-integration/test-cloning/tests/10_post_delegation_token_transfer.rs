...
fn test_post_delegation_action_executes_spl_token_transfer_100() {
    ...
    ctx.airdrop_chain(&authority.pubkey(), 2_000_000_000)
        .unwrap();
    ...
    let assign_ix =
        system_instruction::assign(&authority.pubkey(), &dlp_api::dlp::id());
    let mut delegate_tx = Transaction::new_with_payer(
        &[assign_ix, delegate_with_actions_ix],
        Some(&authority.pubkey()),
    );
    let (_sig, confirmed) = ctx
        .send_and_confirm_transaction_chain(&mut delegate_tx, &[&authority])
        .unwrap();
    assert!(confirmed);
