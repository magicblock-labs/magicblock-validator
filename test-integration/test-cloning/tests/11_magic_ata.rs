...
/// An empty Magic ATA remains authoritative despite a later eATA delegation.
/// Projection is allowed only after the owner explicitly closes the local ATA.
#[test]
fn test_magic_ata_requires_close_before_eata_projection() {
    init_logger!();
    let ctx = IntegrationTestContext::try_new().unwrap();
    ...
    // A subscription update must not replace an existing Magic ATA, even empty.
    ...
    // With the local Magic ATA closed, Chainlink may install the projection.
    let deadline = Instant::now() + Duration::from_secs(20);
    while token_balance_ephem(&ctx, &destination_ata) != Some(EATA_DEPOSIT) {
        assert!(
            Instant::now() < deadline,
            "closed Magic ATA was not replaced by the eATA projection"
        );
        sleep(Duration::from_millis(250));
    }
    assert!(
...
/// local balance (the projection is deferred until it is closed), so callers
