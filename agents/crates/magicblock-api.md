...
- `agents/03_architecture.md` startup path and service boundaries;
...
- SVM/account access validation docs in `agents/02_specification.md`.
...
```bash
git status --short
ls agents/crates/magicblock-api.md
```

Also verify `AGENTS.md` and `agents/04_crate-map.md` mention the new guide when appropriate. Do not run `rs-check` for markdown-only crate-guide updates unless source files were changed.
...
When isolating integration tests, use the setup targets from `agents/05_testing-and-validation.md`, especially `make setup-magicblock-api-both` or the narrower devnet/ephem setup that matches the changed flow.
...
- `agents/00_overview.md` — validator purpose and core lifecycle concepts.
- `agents/02_specification.md` — protocol lifecycle, commit/undelegation, RPC/router, startup/shutdown, and recovery behavior.
- `agents/03_architecture.md` — cross-crate service graph and startup/shutdown boundaries.
- `agents/04_crate-map.md` — crate ownership map and pointer back to this guide.
- `agents/05_testing-and-validation.md` — validation command selection and integration-test setup.
- `agents/crates/magicblock-aperture.md` — RPC/pubsub service details wired by this crate.
- `agents/crates/magicblock-account-cloner.md`, `agents/crates/magicblock-accounts.md`, and `agents/crates/magicblock-chainlink.md` — account sync and scheduled-commit dependencies wired by this crate.
