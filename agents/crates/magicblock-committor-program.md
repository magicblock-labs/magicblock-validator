...
Before handing off any code change, also run the broader baseline from `agents/05_testing-and-validation.md` when practical:
...
- `agents/02_specification.md` — commit, undelegation, committor service, and buffer-task behavior.
- `agents/03_architecture.md` — base-layer settlement layer and service boundaries.
- `agents/04_crate-map.md` — crate ownership and dependency map.
