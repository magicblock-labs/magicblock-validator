...
```bash
test -f agents/crates/magicblock-accounts-db.md
grep -n "magicblock-accounts-db.md" AGENTS.md agents/04_crate-map.md
```
...
Before handing off any Rust change, follow the workspace baseline in `agents/05_testing-and-validation.md` when practical:
...
- `agents/01_validator-goals.md` for security, correctness, performance, and persistence goals.
- `agents/02_specification.md` for account cloning, execution, snapshots/recovery, and writable-account invariants.
- `agents/03_architecture.md` for local persistence and execution/account-sync boundaries.
