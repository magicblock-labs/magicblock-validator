...
```bash
ls agents/crates/magicblock-account-cloner.md
rg "magicblock-account-cloner.md" AGENTS.md agents/04_crate-map.md
```
...
- `agents/02_specification.md` — account cloning, delegated account representation, program cloning, and Magic Program clone instruction notes.
- `agents/03_architecture.md` — account synchronization layer and service boundaries.
- `agents/04_crate-map.md` — crate ownership map and pointer back to this guide.
- `agents/05_testing-and-validation.md` — repository validation commands and integration-test workflow.
- `agents/crates/magicblock-chainlink.md` — Chainlink-side account synchronization, request construction, delegation, subscription, and fetch/clone pipeline guidance.
