...
The RPC-client metrics live in `magicblock-metrics` and are exported with the `mbv_` namespace. Metric names and meanings are operator-facing; changing them requires updating `agents/crates/magicblock-metrics.md` and handoff notes.
...
- `agents/crates/magicblock-metrics.md` for metric documentation expectations.
...
```bash
git diff --check
rg "magicblock-rpc-client.md|magicblock-rpc-client" AGENTS.md agents/04_crate-map.md agents/crates/magicblock-rpc-client.md
```
...
For broader Rust validation, follow `agents/05_testing-and-validation.md`:
...
- `agents/02_specification.md` for base-layer commit/undelegation and RPC/router expectations.
...
- `agents/05_testing-and-validation.md` for required validation workflow.
- `agents/crates/magicblock-metrics.md` for the metrics emitted by `SignatureConfirmer`.
- `agents/crates/magicblock-account-cloner.md`, `agents/crates/magicblock-api.md`, and future committor/table-mania guides for major consumer flows.
