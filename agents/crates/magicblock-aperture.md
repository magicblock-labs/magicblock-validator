...
- `agents/02_specification.md` for protocol-level methods like delegation, routing, or commits.
...
If isolating PubSub tests, use the workflow in `agents/05_testing-and-validation.md`, for example:
...
- `agents/02_specification.md` — RPC/router, account cloning, transaction execution, and lifecycle invariants.
- `agents/03_architecture.md` — cross-crate service boundaries and ingress/event-processing role.
- `agents/04_crate-map.md` — workspace crate ownership map and pointer back to this guide.
- `agents/05_testing-and-validation.md` — repository validation workflow and integration-test commands.
- `agents/06_agent-memory-and-docs.md` — rules for keeping crate guides current.
- `agents/crates/magicblock-account-cloner.md` — account/program clone behavior used by Aperture through Chainlink.
- `agents/crates/magicblock-chainlink.md` — account synchronization and ensure behavior used by RPC reads/transactions.
