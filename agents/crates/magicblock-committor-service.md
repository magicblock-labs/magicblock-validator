...
Start with `src/transaction_preparator/mod.rs` and `delivery_preparator.rs`, then inspect `agents/crates/magicblock-committor-program.md`, `agents/crates/magicblock-table-mania.md`, and `agents/crates/magicblock-rpc-client.md`. Check buffer init/realloc/write chunking, retry handling for already-initialized buffers, cached blockhash invalidation, ALT finalized waits, cleanup-on-success only, and release of TableMania refs.
...
Start with metric calls in `intent_execution_engine.rs`, `delivery_preparator.rs`, and `intent_execution_client.rs`, plus definitions in `magicblock-metrics/src/metrics/mod.rs`. Keep labels low-cardinality (`intent_kind`, `error_kind`, task labels) and update `agents/crates/magicblock-metrics.md` if names/labels change.
...
```bash
git diff --check
rg "magicblock-committor-service.md|magicblock-committor-service" AGENTS.md agents/04_crate-map.md agents/crates/magicblock-committor-service.md
```
...
Before handing off Rust behavior changes, run the broader baseline from `agents/05_testing-and-validation.md` when time allows:
...
- `agents/00_overview.md` for validator runtime context.
- `agents/01_validator-goals.md` for security, settlement, recovery, and performance goals.
- `agents/02_specification.md` for commit, undelegation, Magic Actions, committor pipeline, and recovery behavior.
- `agents/03_architecture.md` for the base-layer settlement layer and service boundaries.
- `agents/04_crate-map.md` for crate ownership and consumers.
- `agents/05_testing-and-validation.md` for workspace and integration validation expectations.
- `agents/crates/magicblock-committor-program.md` for buffer/chunks on-chain helper contracts.
- `agents/crates/magicblock-rpc-client.md` for base-layer send/confirm and RPC helper behavior.
- `agents/crates/magicblock-table-mania.md` for ALT lifecycle and finalized-read semantics.
- `agents/crates/magicblock-accounts.md` for scheduled commit processing and pending-intent recovery call sites.
