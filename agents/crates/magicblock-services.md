...
```bash
git diff -- agents/crates/magicblock-services.md agents/04_crate-map.md AGENTS.md
```
...
Before handoff, run or justify skipping the broader baseline from `agents/05_testing-and-validation.md`:
...
- `agents/02_specification.md` for Magic Actions, commit/undelegation, committor, and callback-related protocol context.
- `agents/03_architecture.md` for service boundaries and base-layer settlement architecture.
- `agents/04_crate-map.md` for workspace crate ownership and consumers.
- `agents/05_testing-and-validation.md` for baseline validation commands.
- `agents/crates/magicblock-core.md` for `ActionsCallbackScheduler`, `BaseActionCallback`, and related shared trait/type contracts.
- `agents/crates/magicblock-api.md` for validator startup and service wiring.
- `agents/crates/magicblock-magic-program-api.md` for callback instruction and response wire types.
- `agents/crates/magicblock-rpc-client.md` if callback delivery begins using shared send/confirm helpers.
