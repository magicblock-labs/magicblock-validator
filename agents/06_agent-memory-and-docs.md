...
- a new crate-specific area that needs its own guide under `agents/crates/`;
...
- `agents/01_validator-goals.md` for goals, correctness constraints, and decision criteria.
- `agents/02_specification.md` for protocol-level behavior and lifecycle rules.
- `agents/03_architecture.md` for cross-crate service interactions and boundaries.
- `agents/04_crate-map.md` for crate ownership, dependencies, consumers, and where to start.
- `agents/05_testing-and-validation.md` for validation commands, debugging workflows, and test selection.
- `agents/crates/<crate>.md` for crate-specific behavior, APIs, invariants, pitfalls, or tests.

If no suitable document exists, create a new focused file in `agents/` or `agents/crates/`. When adding, removing, renaming, or reorganizing agent documentation, update `AGENTS.md` so the entrypoint remains accurate.
