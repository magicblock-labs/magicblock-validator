...
use crate::{
    cpi::helpers::{collect_account_infos, remaining_to_metas},
    InitDelegationRecordArgs,
};

/// CPI helper for `InitDelegationRecord` (on-chain callers).
///
/// ### Accounts:
///
...
#[derive(Clone, Debug)]
pub struct InitDelegationRecordCpi<'a> {
...
impl<'a> InitDelegationRecordCpi<'a> {
    pub fn invoke(&self) -> ProgramResult {
        self.invoke_signed(&[])
    }

    pub fn invoke_signed(&self, signers: &[&[&[u8]]]) -> ProgramResult {
        let ix = crate::builders::InitDelegationRecordBuilder {
