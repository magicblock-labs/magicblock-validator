...
Before a GitHub push, run only tests targeted to the touched behavior. When
fixing an existing pull request, run exactly one relevant unit test or one
relevant integration test; broader validation belongs in CI.
...
- any residual risk;
- whether a durable documentation follow-up was queued for the weekly
  maintenance task, or why none was needed.
