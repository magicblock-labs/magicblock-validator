...
- defers the whole action when a writable dependency is still undelegating after the gate (`DelegationActionDependenciesUndelegating`): the target clone is aborted without rescue-undelegation, so the delegation and its intended action can be retried after the dependency completes undelegation (post-delegation exactly-once invariant), instead of executing against a locked account and being undelegated with no retry path. Covered by `test_post_delegation_actions_never_refresh_undelegating_dependency`,
