...
- Every delegation-record-shaped program update records a sighting (`record pubkey -> slot`) before being dropped. Sightings are monotonic (max slot): a stale record update replayed out of order can neither lower a sighting nor release a parked candidate whose slot it does not cover.
