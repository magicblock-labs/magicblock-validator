...
- never force-refreshes a dependency that is locally undelegating: such keys are routed through `should_refresh_undelegating_in_bank_account` (with the record resolved at the freshest of the caller's requested slot and the latest observed chain slot, matching the account-fetch max-slot rule, so classification is slot-coherent per the delegation slot-consistency invariant), and may only be overridden after a confirmed undelegation or a newer delegation slot. Covered by `test_post_delegation_actions_never_refresh_undelegating_dependency`,
