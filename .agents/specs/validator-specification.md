...
Chainlink uses `Engine::account(pubkey).create(...)` only for ensure/fetch
materialization and `update(...)` for subscription refreshes of accounts already
present locally. Creation composes ordered MagicRoot patches and finalization,
followed by `PostFinalize` when a delegation record carries actions; updates
never carry post-delegation actions. The same create/update distinction applies
to program accounts, which are always read-only and never delegated.

Every Chainlink materialization has one explicit mode: a valid local delegation
is `Delegated`, a confined account is zero-lamport `Ephemeral`, a newly
materialized zero-lamport account is `Placeholder`, and every remaining remote
account is `ReadOnly`. Chainlink must reject all other engine account modes.
