...
pub(crate) fn process_schedule_intent_bundle(
    ...
    let (undelegated_pubkeys, scheduled_intent) = {
        ...
        let undelegated_accounts_ref =
            ...
            } else if let Some(ref value) = args.commit_finalize_and_undelegate {
                Some(CommitType::extract_commit_accounts(
                    value.committed_accounts_indices(),
                    construction_context.transaction_context(),
                )?)
        ...
        let undelegated_pubkeys =
            if let Some(undelegated_accounts_ref) =
                undelegated_accounts_ref.as_ref()
            {
