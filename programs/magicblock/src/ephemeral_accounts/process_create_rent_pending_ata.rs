...
pub(crate) fn process_create_rent_pending_ata(
    ...
    let mint_data_len = {
        ...
        token_account_len(mint_acc.data(), &token_program)?
    };

    let mut acc = ata.borrow_mut()?;
    acc.set_lamports(0);
    acc.set_owner(token_program);
    acc.resize(mint_data_len, 0);
...
fn initialize_token_data(
    ...
    if *token_program == TOKEN_PROGRAM_ID {
        ...
        let mut state =
            StateWithExtensionsMut::<Token2022Account>::unpack(data)
                .map_err(|_| InstructionError::InvalidAccountData)?;
